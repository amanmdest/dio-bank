...
async def create_account(account: AccountIn):
    return await {**account.model_dump(), 'id': services.create(account)}


@router.get('/', response_model=AccountOut, status_code=status.HTTP_200_OK)
...
async def update_account(account: AccountPut, id: int):
    return await services.update(id)


@router.delete('/{id}', response_model=AccountOut, status_code=status.HTTP_200_OK)
